...
export class FormingBarManager extends EventEmitter {
  ...
  async #seedRollover1mPartial(nowSec, timeoutMs, marker) {
    try {
      ...
      if (isUsablePrice(tsOpen)) {
        ...
        const high = from1s
          ? Math.max(Number(from1s.high), tsOpen)
          : tsOpen;
        const low = from1s
          ? Math.min(Number(from1s.low), tsOpen)
          : tsOpen;
        const close = from1s ? Number(from1s.close) : tsOpen;
        this.#partial1m = {
          ...(from1s ?? {}),
          marker,
          open: tsOpen,
        ...
        log1mBuild("seedRollover1mPartial", this.#partial1m, {
          openFrom: `TradeSea 1s open (${bars1s.length} bars)`,
          bars1s,
        });
        return;
      }
  ...
  async #refineRollover1m(nowSec, timeoutMs, targetOpen) {
    try {
      await this.#buildPartial1mFrom1s(nowSec, timeoutMs, targetOpen);
      if (!this.#partial1m || Number(this.#partial1m.marker) !== targetOpen) return;
      this.#publishPartial1mToForming(nowSec, targetOpen);
      this.#sync1mCloseFromLast();
      log1m("refineRollover1m.done", fmt1mBar(this.#forming.get(resolutionKey(1))));
    } catch (err) {
  ...
  #maybeRefine1mOpenFrom1s() {
    if (!this.#accuracyMode || this.#refine1mOpenInflight) return;
    const nowSec = Math.floor(Date.now() / 1000);
    const marker = bucketOpen(nowSec, ONE_MINUTE_PERIOD);
  ...
  async #refine1mOpenFrom1s(marker, nowSec) {
    try {
      ...
      const tsOpen = tradeseaMinuteOpenFrom1s(bars1s, marker);
      if (!isUsablePrice(tsOpen)) return;
      ...
      if (Math.abs(tsOpen - curOpen) < 0.001) return;

      const next = {
        ...bar,
        open: tsOpen,
        high: Math.max(Number(bar.high), tsOpen),
        low: Math.min(Number(bar.low), tsOpen),
        forming: true,
        replaySource: String(bar.replaySource ?? "1m").includes("ts-open")
          ? bar.replaySource
          : `${bar.replaySource ?? "1m"}+ts-open-refine`,
      };
      ...
      this.#forming.set(key, next);
      log1m("refine1mOpenFrom1s", `${curOpen} → ${tsOpen}`);
      log1mBuild("refine1mOpenFrom1s", next, {
        openFrom: `TradeSea :01 from ${sorted.length} 1s bars`,
        openWas: curOpen,
        openNow: tsOpen,
        bars1s: sorted,
      });
