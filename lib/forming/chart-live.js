...
export class ChartLive extends EventEmitter {
  ...
  constructor(chart, mgr, supervisor, options) {
    ...
    this.#formingEnabled = options.forming !== false;
  ...
  async start() {
    if (this.#formingEnabled) {
      ...
      this.#log("forming disabled — skipping bootstrap");
    }

    const symbol = this.#chart.symbol ?? "?";
    this.#log(
      `Symbol: ${symbol}  Timeframe: ${this.#resolution} min  Compat: ${this.#compat ? "Enabled" : "Disabled"}  Forming: ${this.#formingEnabled ? "On" : "Off"}`,
    );
  ...
  #onTimeBar(tb) {
    ...
    if (this.#formingEnabled) void this.#roll(endMarker, tb);
  }
