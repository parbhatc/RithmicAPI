...
/** 1s sub-bars that count for TradeSea forming H/L (skip boundary :00 when :01 open exists). */
export function tradeseaActive1sBars(bars1s, marker, periodSeconds = 60) {
  if (!bars1s?.length) return [];
  const sorted = [...bars1s].sort(
    (a, b) => Number(a.marker) - Number(b.marker),
  );
  let startIdx = 0;
  if (sorted.length > 1 && Number(sorted[0].marker) === Number(marker)) {
    startIdx = 1;
  }
  return sorted
    .filter((b) => {
      const t = Number(b.marker);
      return t >= marker && t < marker + periodSeconds;
    })
    .slice(startIdx);
}

/** TradeSea 1m forming OHLC from 1s bars — open from :01, H/L/C from same window (no :00 wick). */
export function tradeseaMinuteFormingFrom1s(bars1s, marker, periodSeconds = 60) {
  const open = tradeseaMinuteOpenFrom1s(bars1s, marker);
  if (!isUsablePrice(open)) return null;

  const active = tradeseaActive1sBars(bars1s, marker, periodSeconds);
  if (!active.length) {
    return { open, high: open, low: open, close: open };
  }

  const agg = aggregateReplayOHLC(active, { marker, periodSeconds });
  if (!agg) return { open, high: open, low: open, close: open };

  const high = Number(agg.high);
  const low = Number(agg.low);
  const close = Number(agg.close);
  return {
    open,
    high: isUsablePrice(high) ? high : open,
    low: isUsablePrice(low) ? low : open,
    close: isUsablePrice(close) ? close : open,
  };
}
