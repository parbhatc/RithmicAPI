...
export function lastTradeseaBar(payload) {
  ...
  return barFromTradeseaPayload(payload, i);
}

/** Row at chart time `t` (bucket open unix sec). */
export function tradeseaBarAtTime(payload, chartTimeSec) {
  if (!payload?.t?.length) return null;
  const t = Number(chartTimeSec);
  const i = payload.t.findIndex((x) => Number(x) === t);
  if (i < 0) return null;
  return barFromTradeseaPayload(payload, i);
}

function barFromTradeseaPayload(payload, i) {
  return {
    time: payload.t[i],
    marker: payload.t[i],
    open: payload.o[i],
