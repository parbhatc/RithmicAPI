...
export class FormingBarManager extends EventEmitter {
  ...
  onTrade(trade) {
    if (trade?.symbol && trade.symbol !== this.#s.session.symbol) return new Map();
    if (trade?.exchange && trade.exchange !== this.#s.session.exchange) return new Map();
    ...
    for (const [key, periodSeconds] of this.#s.targets) {
      ...
      let seedOpen;
      if (prev && Number(prev.marker) === tradeMarker && isUsablePrice(prev.open)) {
        seedOpen = prev.open;
      }
      const next = applyTradeToFormingBar(prev, trade, {
        periodSeconds,
