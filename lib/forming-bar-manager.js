...
import { fmt1mBar, log1m, log1mBars } from "./forming-1m-debug.js";
import {
 ...
 * |------|----------|----------------------|------|
 * | 1m | 1 | **1s** → 1m + shared 1m fetch | LastTrade |
 * | Minutes+ | 2, 5, 15, 60, 240, 1D | **1 shared** 1m fetch → rollup | LastTrade |
 * | Week/Month | 1W, 1M | **1 shared** 1D fetch → rollup | LastTrade |
 * | Year | 1Y | **1 shared** 1M fetch → rollup | LastTrade |
...
export class FormingBarManager extends EventEmitter {
  ...
  onTrade(trade) {
    ...
    for (const [key, periodSeconds] of this.#targets) {
      ...
      if (this.#accuracyMode && (resolution === 1 || resolution === "1" || periodSeconds === ONE_MINUTE_PERIOD || key === "1")) {
        continue;
      }
  ...
  async #run1mShared(req, nowSec, timeoutMs, tickFallback) {
    ...
    log1m(
      "run1mShared.split",
      `bucket=${new Date(current1mOpen * 1000).toLocaleString()} closed=${split.closed.length} partial=${split.partial ? fmt1mBar(split.partial) : "null"}`,
    );
    if (!this.#partial1m && history1m.length) {
      const last = history1m.at(-1);
      if (Number(last.marker) === current1mOpen) {
        this.#partial1m = last;
        log1m("run1mShared.partialFallback", fmt1mBar(last));
      }
    }
  ...
  async #buildPartial1mFrom1s(nowSec, timeoutMs) {
    ...
    try {
      ...
      this.#partial1m = {
        ...from1s,
        forming: true,
        replaySource: this.#compat1m() ? "1m-1s+compat" : "1m-1s",
      };
      log1m("buildPartial1mFrom1s.ok", fmt1mBar(this.#partial1m));
    } catch (err) {
  ...
  async #ensure1mPartial(nowSec, timeoutMs) {
    const marker = bucketOpen(nowSec, ONE_MINUTE_PERIOD);
    log1m("ensure1mPartial.start", `bucket=${new Date(marker * 1000).toLocaleString()} compat=${this.#compat1m()}`);
    ...
    try {
      ...
      if (split.partial && Number(split.partial.marker) === marker) {
        ...
        log1m("ensure1mPartial.ok", fmt1mBar(this.#partial1m));
        return;
      }
      const last = history1m.at(-1);
      if (last && Number(last.marker) === marker) {
        ...
        log1m("ensure1mPartial.lastBar", fmt1mBar(this.#partial1m));
        return;
      }
    ...
    try {
      ...
      if (from1s) {
        this.#partial1m = {
          ...from1s,
          forming: true,
          replaySource: this.#compat1m() ? "1m-1s-seed+compat" : "1m-1s-seed",
        };
    ...
    let secOpen = null;
    try {
      secOpen = await this.#session.first1sOpenInRange(
        marker,
    ...
    log1m("ensure1mPartial.liveSeedDone", `secOpen=${secOpen ?? "—"} ${fmt1mBar(this.#partial1m)}`);
  }
  ...
  /** @deprecated Native partial fallback — primary path is {@link #runDailyShared}. */
  ...
  #applyLastToFormingCloses(last) {
    for (const [key, bar] of this.#forming) {
      ...
      if (this.#accuracyMode && (resolution === 1 || resolution === "1" || ps === ONE_MINUTE_PERIOD)) {
        this.#forming.set(key, { ...bar, close: last, forming: true });
        continue;
      }
  ...
  async refreshCurrent1m(nowSec = Math.floor(Date.now() / 1000), timeoutMs = 8000) {
    const current1mOpen = bucketOpen(nowSec, ONE_MINUTE_PERIOD);
    log1m(
      "refreshCurrent1m.start",
      `now=${new Date(nowSec * 1000).toLocaleString()} bucket=${new Date(current1mOpen * 1000).toLocaleString()}`,
    );

    if (this.#accuracyMode) {
      const existing = this.#forming.get(resolutionKey(1));
      if (existing && Number(existing.marker) === current1mOpen) {
        log1m("refreshCurrent1m.path", "close-only (forming bar already on bucket)");
        this.#sync1mCloseFromLast();
        log1m("refreshCurrent1m.final", fmt1mBar(this.#forming.get(resolutionKey(1))));
        return;
      }
      const havePartial =
        this.#partial1m && Number(this.#partial1m.marker) === current1mOpen;
      log1m(
        ...
        havePartial
          ? `publish partial=${fmt1mBar(this.#partial1m)}`
          : "ensure1mPartial (no partial for bucket)",
      );
      if (!havePartial) await this.#ensure1mPartial(nowSec, timeoutMs);
      this.#publishPartial1mToForming(nowSec);
      this.#sync1mCloseFromLast();
      log1m("refreshCurrent1m.final", fmt1mBar(this.#forming.get(resolutionKey(1))));
      return;
    ...
    if (!havePartial) {
      try {
        ...
        if (!partial && history1m.length) {
          const last = history1m.at(-1);
          if (Number(last.marker) === current1mOpen) partial = last;
        }
        if (partial) {
          this.#partial1m = {
            ...partial,
  ...
  /**
   * Accuracy 1m open: history TimeBar open; first-1s when unset or off by ≥0.25 pt (TradeSea).
   */
  async #refinePartial1mAccuracy(nowSec, timeoutMs) {
    ...
    let open = histOpen;
    if (isUsablePrice(secOpen)) {
      if (!isUsablePrice(histOpen)) open = secOpen;
      else if (Math.abs(secOpen - histOpen) >= 0.24) open = secOpen;
    }
    ...
    const used1s =
      isUsablePrice(secOpen) && Math.abs(open - secOpen) < 0.001;
    this.#partial1m = {
      ...this.#partial1m,
      open,
      high: h,
      low: l,
      forming: true,
      replaySource: used1s ? "1m-partial+1s-open" : "1m-partial",
    };
  }

  /** Accuracy 1m: history partial O/H/L + live last for close only. */
  #sync1mCloseFromLast() {
    const key = resolutionKey(1);
    const bar = this.#forming.get(key);
    const last = Number(this.#session.status?.last);
    if (bar && isUsablePrice(last)) {
      const prev = bar.close;
      this.#forming.set(key, { ...bar, close: last, forming: true });
      log1m("sync1mCloseFromLast", `close ${prev} → ${last}`);
    } else {
  ...
  #publishPartial1mToForming(nowSec) {
    ...
    const current1mOpen = bucketOpen(nowSec, ONE_MINUTE_PERIOD);
    if (Number(this.#partial1m.marker) !== current1mOpen) {
      log1m(
        "publishPartial1m.skip",
        `marker mismatch partial=${new Date(Number(this.#partial1m.marker) * 1000).toLocaleString()} bucket=${new Date(current1mOpen * 1000).toLocaleString()}`,
      );
      return;
    }
    ...
    this.#forming.set(key, bar);
    log1m("publishPartial1m.ok", fmt1mBar(bar));
   ...
   * Native-1h high override removed — it diverged from TradeSea on LucidTrading.
   */
