...
export class FormingBootstrapCache {
  ...
  oneMinuteKey(session, from, countback, nowSec) {
    return `${symKey(session)}:1m:${from}:${countback}:${Math.floor(nowSec / 60)}`;
  }

  /**
   * @returns {{ raw: object[], closed: object[], partial: object|null }|null}
   */
  get1m(session, from, countback, nowSec, ttlMs = DEFAULT_1M_TTL_MS) {
    const key = this.oneMinuteKey(session, from, countback, nowSec);
  ...
  set1m(session, from, countback, nowSec, raw, closed, partial) {
    const key = this.oneMinuteKey(session, from, countback, nowSec);
