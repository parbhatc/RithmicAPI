...
export class ReceiveYield extends Error {
  constructor(label = "Rithmic") {
    super(`${label}: receive yielded for replay`);
    this.name = "ReceiveYield";
  }
}

/** Drop oldest frames when the live pump falls behind (prevents OOM). */
const MAX_RECV_QUEUE = 2000;
...
export class Client {
  ...
  async connect() {
    ...
    this.ws.on("message", (data, isBinary) => {
      ...
      if (this.#recvWaiters.length) {
        this.#recvWaiters.shift()(frame);
        return;
      }
      while (this.#recvQueue.length >= MAX_RECV_QUEUE) {
    ...
    this.ws.on("close", (code, reason) => {
      this.wireSniffer?.close?.(this.label, {
        code,
    ...
    this.ws.on("error", (err) => {
      this.wireSniffer?.error?.(this.label, err?.message ?? String(err));
    });

    this.wireSniffer?.open?.(this.label, this.uri);
  ...
  send(packet) {
    ...
    this.#wireLog("send", packet, frame);
    this.ws.send(frame);
    return packet;
  }

  /** Wake blocked receive() loops so replay can own the socket (live pump). */
  releaseReceiveWaiters() {
    const waiters = this.#recvWaiters.splice(0);
  ...
  close() {
    ...
    this.ws = null;
    this.#recvQueue = [];
    this.#queueDrainReads = 0;
    this.#pendingPackets = [];
    this.releaseReceiveWaiters();
  }

  async #receiveFromWire() {
    const raw = await this.#readFrame();
    if (raw === undefined) {
      throw new ReceiveYield(this.label);
    }
    const packet = Packet.decodeFrame(raw);
    this.#wireLog("recv", packet, parseFrame(raw).frame);
    return packet;
  }

  #wireLog(dir, packet, frame) {
    const wireDir = dir === "send" ? "SENT" : "RECEIVED";
    this.wireSniffer?.packet?.(this.label, wireDir, packet);
    if (!this.log) return;
  ...
  #readFrame() {
    return new Promise((resolve, reject) => {
      if (this.#recvQueue.length) {
        ...
        // Yield periodically when draining a backlog — avoids microtask stack overflow.
        if (++this.#queueDrainReads % 32 === 0) {
          setImmediate(() => resolve(frame));
        } else {
          resolve(frame);
        }
        return;
