...
/**
 * Canonical store:
 * - Completed HTF bars → native Rithmic replay (`loadHistory` at that resolution)
 * - Open HTF bucket → 1m replay from bucket open → now only, then rollup
 * - Live → ticks update forming 1m only
 */
export class CandleLayer {
  ...
  /** @type {Map<number, number>} HTF periodSeconds → tick open at bucket start minute */
   ...
   * @param {(number|string)[]} [options.alsoFor] e.g. `[15]` → native 15m completed + 1m tail for 4:30+
  ...
  async load1m({
    ...
    if (tickOpen && this.#tail1mFrom != null) {
      ...
      refinedTail = await this.#session.replay1mBarsFromTicks(
        this.#tail1mFrom,
        nowSec + 120,
        { timeoutMs },
      );
      timings.refine1mTailFromTicks_ms = Math.round(performance.now() - tRefine);
