...
export function parseResolution(resolution = 1) {
  ...
  if (raw === "Y" || raw === "1Y" || raw === "12M" || raw === "YEARLY") {
    return { barType: BarType.DAILY_BAR, barTypePeriod: 365, periodSeconds: 365 * 86_400 };
  }
...
/**
 * Shift OHLCV forward one bar (TradingView / Auren compat): label time, next bar's values.
 * @param {object[]} bars — sorted ascending
 * @returns {object[]}
 */
export function applyCompatBars(bars) {
  ...
  for (let i = 0; i + 1 < bars.length; i++) {
    ...
    const marker = valueBar.forming ? valueT : gap > step ? valueT - step : labelT;
    ...
    out.push({
      ...
      forming: valueBar.forming,
      period: valueBar.period ?? labelBar.period,
    });
  }
  return out;
}
...
export function barsToHistoryPayload(bars, { timeOffset = 0, compat = false } = {}) {
  ...
  if (compat) {
    for (const bar of applyCompatBars(bars)) {
      t.push(barTimeSec(bar) + timeOffset);
      o.push(bar.open);
      h.push(bar.high);
      l.push(bar.low);
      c.push(bar.close);
